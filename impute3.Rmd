...
dfl <- lapply(unique(gs_gx$ind), function (f) {
  mypos <- gs_gx[gs_gx$ind == f,3]
  mypos2 <- paste(mypos,collapse=",")
  COMMAND <- paste("cut -f",mypos2, cdatlist[f],"> tmp")
  cx <- read.table("tmp",header=TRUE)
  return(cx)
})
...
pl <- list("out"=pcout,"in"=pcin)
vioplot(pl,main=gsname,ylab="Pearson correlation coefficient")

mid_in <- mean(c(mean(pcin),median(pcin)))
mid_out <- mean(c(mean(pcout),median(pcout)))

# now test each other gene
i=1
gm <- unlist(lapply(1:nrow(cmx), function(i) {
...
hist(gm,xlab="mid Pearson correlation values")
abline(v=c(mid_in,mid_out),col=c("red","blue"),lty=2,lwd=2)

ensg[match(names(candidates),ensg$ensembl_gene_id),]
